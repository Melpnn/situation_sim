...
function App() {
  ...
  // NEW: This state tracks where the player avatar is currently located
  ...
  const handleChoice = (damage, response, actionType) => {
    ...
    setPlayerAction(actionType); // This tells Framer Motion to move the player
    
    if (damage === 0) {
      setIsShaking(false);
    } else {
      // Screen shakes violently if the player takes damage
      setIsShaking(true);
      setTimeout(() => setIsShaking(false), 800);
    }
  };

  // NEW: The exact coordinates and colors for the Player Avatar's movements
  const playerVariants = {
    idle: { x: 0, y: 0, backgroundColor: "#3b82f6", scale: 1 }, // Blue, center
    desk: { x: -100, y: 40, backgroundColor: "#22c55e", scale: 0.85 }, // Green, safe, shrunk down (hiding)
    stairs: { 
      x: 120, y: -40, 
      backgroundColor: "#ef4444", 
      scale: 1.1,
      rotate: [0, -20, 20, -20, 0], // The avatar physically shakes when hit
      transition: { rotate: { repeat: 3, duration: 0.15 } }
    } 
  };

  return (
    <div className={`h-screen w-screen bg-slate-950 text-white overflow-hidden flex items-center justify-center font-sans ${isShaking ? 'animate-shake' : ''}`}>
      ...
      <AnimatePresence mode="wait">
        {gameState === 'title' ? (
          <motion.div key="title" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -20 }} className="text-center z-10">
            ...
            <button onClick={startGame} className="bg-red-600 hover:bg-red-500 px-16 py-6 rounded-sm font-black text-3xl uppercase italic flex items-center gap-4 mx-auto transition-all">
          ...
          <div className="w-full h-full flex flex-col items-center p-8 relative">
            ...
            <div className="flex flex-col items-center justify-center flex-grow w-full max-w-4xl mt-10">
              ...
              <h2 className="text-5xl font-black uppercase tracking-tighter mb-4 text-center">Seismic Event Detected</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-3xl">
                {/* Passing 'desk' as the action type */}
                <ChoiceCard 
                  text="Secure Position Under Desk" 
                  onSelect={() => handleChoice(0, "Survival secured. Structure remains intact.", 'desk')} 
                />
                {/* Passing 'stairs' as the action type */}
                <ChoiceCard 
                  text="Evacuate Via Stairwell" 
                  onSelect={() => handleChoice(50, "Catastrophic failure. Debris has caused major trauma.", 'stairs')} 
                />
              </div>
            </div>
          </div>
